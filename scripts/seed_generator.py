    ...
    for mid, name, era, region, principle, signal in MOVEMENTS:
        for med_id, med_name, med_principle in MEDIA:
            ...
            add_seed(
                ...
                era,
                region,
                f"{principle};{med_principle}",
                f"{signal},{med_name}排版",
                3,
            )
    ...
    for mid, name, era, region, principle, signal in MOVEMENTS:
        for mood_id, mood_name, mood_colors in MOODS:
            ...
            add_seed(
                ...
                era,
                region,
                f"{principle};{mood_name}色盘",
                f"{signal},{mood_colors}",
                4,
            )
    ...
    for mid, name, era, region, principle, signal in MOVEMENTS:
        for med_id, med_name, med_principle in MEDIA:
            for mood_id, mood_name, mood_colors in MOODS:
                ...
                add_seed(
                    sid,
                    f"{name}·{med_name}·{mood_name}",
                    era,
                    region,
                    f"{principle};{med_principle};{mood_name}色盘",
                    f"{signal},{mood_colors},{med_name}排版",
                    6,
                )
    ...
    for mid, name, era, region, principle, signal in MOVEMENTS:
        for tex_id, tex_name, tex_desc in TEXTURES:
            for mood_id, mood_name, mood_colors in MOODS:
                ...
                add_seed(
                    sid,
                    f"{name}·{tex_name}·{mood_name}",
                    era,
                    region,
                    f"{principle};{tex_desc};{mood_name}色盘",
                    f"{signal},{mood_colors},{tex_desc}",
                    7,
                )
...
def write_seeds_md(seeds, path):
    ...
    for tier in sorted(by_tier.keys()):
        ...
        for s in tier_seeds:
            lines.append(
                f"| `{s['id']}` | {s['name']} | {s['era']} | {s['region']} | {s['principle']} | {s['anti_ai_signal']} |"
            )
        lines.append("")
